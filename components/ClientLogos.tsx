    ...
    <section className="bg-white py-16 text-black">
      <div className="container mx-auto px-4 text-center">
        ...
        <div className="relative flex items-center justify-center gap-4 md:gap-8 flex-wrap">
          {/* Left Arrow */}
          <button className="hidden md:block absolute left-0 top-1/2 -translate-y-1/2 p-2 text-black hover:text-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 items-center justify-items-center w-full max-w-5xl px-8">
            {clients.map((client, index) => (
              <div key={index} className="opacity-70 hover:opacity-100 transition-opacity">
                ...
                <div className="text-xl font-bold text-black border border-black p-4 rounded min-w-[120px] text-center">
                  {client}
                </div>
              </div>
            ))}
          ...
          {/* Right Arrow */}
          <button className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 p-2 text-black hover:text-white">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10">
